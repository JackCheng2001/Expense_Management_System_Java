...
public class ExpenseFileParser {
	...
	public static List<Map<Integer, Double>> parseExpenses(List<String> expenseList) {
		...
		List<Map<Integer, Double>> parsedExpenses = new ArrayList<>();	// Initialize an empty list to store the parsed expenses.
		// Iterate over each line in the provided list of expense strings.
		for (String line : expenseList) {
			line = line.trim();    // Trim leading and trailing whitespace from the line.
			// Check if the line is not empty after trimming.
			if (!line.isEmpty()) {
				// Split the line based on delimiters (comma, colon, or spaces).
				// The regex "[,: ]+" matches one or more of any of the characters: comma, colon, or space.
				String[] parts = line.split("[,: ]+");
				// Check if the split resulted in exactly two parts (month and amount).
				if (parts.length == 2) {
					try {
						// Parse the first part as the month (integer) and the second part as the expense amount (double).
						int month = Integer.parseInt(parts[0]);
						double amount = Double.parseDouble(parts[1]);
						// Create a new map to store the month and amount as a key-value pair.
						Map<Integer, Double> expenseMap = new HashMap<>();
						expenseMap.put(month, amount);
						// Add the created map to the list of parsed expenses.
						parsedExpenses.add(expenseMap);
					} catch (NumberFormatException e) {
						// If parsing the month or amount fails, print an error message and skip this line.
